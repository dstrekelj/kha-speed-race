...
Main.__name__ = true;
Main.main = function() {
	kha_System.init({ title : "kha-speed-race", width : 480, height : 640},Main.init);
};
Main.init = function() {
	kha_Assets.loadEverything(Main.postLoadEverything);
};
Main.postLoadEverything = function() {
	var g = new game_SpeedRace();
	kha_System.notifyOnRender($bind(g,g.render));
	kha_Scheduler.addTimeTask($bind(g,g.update),0,0.016666666666666666);
};
Math.__name__ = true;
...
var game_SpeedRace = function() {
	kha_math_Random.init(160619);
	if(kha_input_Keyboard.get() != null) {
		kha_input_Keyboard.get().notify($bind(this,this.onKeyDown),$bind(this,this.onKeyUp));
	}
	this.isRunning = false;
	this.velocity = 0;
	this.backbuffer = kha_Image.createRenderTarget(240,320);
	this.player = new game_sprites_PlayerCar(kha_Assets.images.car_big);
	var _this = this.player;
	_this.x = 120. - _this.width / 2;
	_this.y = 213.33333333333334 - _this.height / 2;
	this.cars = new game_tools_Pool(10);
	var _g = 0;
	while(_g < 10) {
		++_g;
		this.cars.add(new game_sprites_NpcCar(0,0,kha_Assets.images.car_small)).kill();
	}
	this.road = new game_groups_Road(20,kha_Assets.images.road_segment);
	game_sprites_Car.X_MIN = 20;
	game_sprites_Car.X_MAX = 220;
	this.score = 0;
	this.scoreText = new game_tools_ui_Text(kha_Assets.fonts.slkscr,64);
	this.instructions = new game_tools_ui_Text(kha_Assets.fonts.slkscr,10);
	this.instructions.set_value("W - SPEED UP / RESTART. S - SPEED DOWN. A - LEFT. D - RIGHT.");
	var _this1 = this.instructions;
	var x = kha_System.windowWidth() / 2;
	var y = kha_System.windowHeight() - this.instructions.height;
	_this1.x = x - _this1.width / 2;
	_this1.y = y - _this1.height / 2;
	kha_Scheduler.addTimeTask($bind(this,this.spawnCar),0,1);
};
$hxClasses["game.SpeedRace"] = game_SpeedRace;
game_SpeedRace.__name__ = true;
game_SpeedRace.prototype = {
	backbuffer: null
	,cars: null
	,instructions: null
	,isRunning: null
	...
	,update: function() {
		...
		if(this.isRunning) {
			this.player.update();
			this.score += this.velocity / 6 / 10;
			this.scoreText.set_value(Std.string(Math.floor(this.score)));
			var _this = this.scoreText;
			var x = kha_System.windowWidth() / 2;
			var y = kha_System.windowHeight() - this.scoreText.height;
			_this.x = x - _this.width / 2;
			_this.y = y - _this.height / 2;
			this.cars.each($bind(this,this.handleCollision));
		}
	}
	,render: function(framebuffer) {
		var g = this.backbuffer.get_g2();
		g.begin(null,kha__$Color_Color_$Impl_$._new(-16724839));
		this.road.draw(g);
		this.player.draw(g);
		this.cars.draw(g);
		g.end();
		framebuffer.get_g2().begin(null,kha__$Color_Color_$Impl_$.Blue);
		...
		this.scoreText.draw(framebuffer.get_g2());
		this.instructions.draw(framebuffer.get_g2());
		framebuffer.get_g2().end();
	}
	,updateVelocity: function(step) {
		var _gthis = this;
		this.velocity += step;
		if(this.velocity < 2 && this.isRunning) {
			this.velocity = 2;
		}
		if(this.velocity < 0) {
			this.velocity = 0;
		...
		this.road.updateVelocity(this.velocity);
		this.cars.each(function(c) {
			c.updateVelocity(_gthis.velocity,6,2);
		});
	}
	,spawnCar: function() {
		if(this.isRunning) {
			var _g1 = 0;
			var _g = kha_math_Random.Default.GetIn(1,3);
			while(_g1 < _g) {
				++_g1;
				var c = this.cars.getFirstDead();
				if(c != null) {
					c.revive();
					c.init(this.velocity,6,2);
				}
			}
		}
	}
	,handleCollision: function(c) {
		if(c.isActive && c.isVisible) {
			if(c.overlapsEntity(this.player)) {
				this.stopGame();
			}
		}
	}
	...
	,stopGame: function() {
		this.isRunning = false;
		this.updateVelocity(-6.);
	}
	,onKeyDown: function(key,$char) {
		switch($char) {
		case "a":
			this.player.updateMovement(this.velocity,-1);
			break;
		case "d":
			this.player.updateMovement(this.velocity,1);
			break;
		case "s":
			this.updateVelocity(-2.);
			break;
		case "w":
			if(!this.isRunning) {
				this.startGame();
	...
	,onKeyUp: function(key,$char) {
		switch($char) {
		case "a":
			this.player.updateMovement(0,-1);
			break;
		case "d":
			this.player.updateMovement(0,1);
			break;
	...
	,__class__: game_SpeedRace
};
var game_tools_Group = function() {
	this.elements = [];
};
$hxClasses["game.tools.Group"] = game_tools_Group;
game_tools_Group.__name__ = true;
game_tools_Group.prototype = {
	elements: null
	,update: function() {
		var _g = 0;
	...
	,__class__: game_tools_Group
};
var game_groups_Road = function(x,graphic) {
	game_tools_Group.call(this);
	var segmentCount = Math.ceil(320 / graphic.get_height());
	var _g1 = -2;
	while(_g1 < segmentCount) this.add(new game_groups__$Road_RoadSegment(x,_g1++ * graphic.get_height(),graphic));
};
$hxClasses["game.groups.Road"] = game_groups_Road;
game_groups_Road.__name__ = true;
game_groups_Road.__super__ = game_tools_Group;
game_groups_Road.prototype = $extend(game_tools_Group.prototype,{
	updateVelocity: function(v) {
		game_groups__$Road_RoadSegment.v = v;
	}
	,__class__: game_groups_Road
});
var game_tools_Object = function(x,y,width,height) {
	this.x = x;
	this.y = y;
	this.width = width;
	this.height = height;
...
$hxClasses["game.tools.Object"] = game_tools_Object;
game_tools_Object.__name__ = true;
game_tools_Object.prototype = {
	x: null
	,y: null
	,width: null
	,height: null
	...
	,__class__: game_tools_Object
};
var game_tools_Entity = function(x,y,width,height) {
	game_tools_Object.call(this,x,y,width,height);
	this.isOverlapping = true;
};
$hxClasses["game.tools.Entity"] = game_tools_Entity;
game_tools_Entity.__name__ = true;
game_tools_Entity.__super__ = game_tools_Object;
game_tools_Entity.prototype = $extend(game_tools_Object.prototype,{
	isOverlapping: null
	,update: function() {
		game_tools_Object.prototype.update.call(this);
	}
	,draw: function(g) {
		game_tools_Object.prototype.draw.call(this,g);
	}
	,overlapsPoint: function(px,py) {
	...
	,__class__: game_tools_Entity
});
var game_tools_Sprite = function(x,y,graphic) {
	game_tools_Entity.call(this,x,y,graphic.get_width(),graphic.get_height());
	this.graphic = graphic;
};
$hxClasses["game.tools.Sprite"] = game_tools_Sprite;
game_tools_Sprite.__name__ = true;
game_tools_Sprite.__super__ = game_tools_Entity;
game_tools_Sprite.prototype = $extend(game_tools_Entity.prototype,{
	graphic: null
	,update: function() {
		game_tools_Entity.prototype.update.call(this);
	}
	,draw: function(g) {
		game_tools_Entity.prototype.draw.call(this,g);
		g.drawImage(this.graphic,this.x,this.y);
	}
	,__class__: game_tools_Sprite
});
var game_groups__$Road_RoadSegment = function(x,y,graphic) {
	game_tools_Sprite.call(this,x,y,graphic);
};
$hxClasses["game.groups._Road.RoadSegment"] = game_groups__$Road_RoadSegment;
game_groups__$Road_RoadSegment.__name__ = true;
game_groups__$Road_RoadSegment.__super__ = game_tools_Sprite;
game_groups__$Road_RoadSegment.prototype = $extend(game_tools_Sprite.prototype,{
	update: function() {
		game_tools_Sprite.prototype.update.call(this);
		if(this.y > 320) {
			this.y = this.y - 320 - 2 * this.height;
		}
		this.y += game_groups__$Road_RoadSegment.v;
	}
	,__class__: game_groups__$Road_RoadSegment
});
var game_sprites_Car = function(x,y,graphic) {
	game_tools_Sprite.call(this,x,y,graphic);
	this.direction = new kha_math_Vector2(0,0);
	this.velocity = 0;
};
$hxClasses["game.sprites.Car"] = game_sprites_Car;
game_sprites_Car.__name__ = true;
game_sprites_Car.__super__ = game_tools_Sprite;
game_sprites_Car.prototype = $extend(game_tools_Sprite.prototype,{
	direction: null
	,velocity: null
	,update: function() {
		game_tools_Sprite.prototype.update.call(this);
		...
		if(this.x < game_sprites_Car.X_MIN) {
			this.x = game_sprites_Car.X_MIN;
		}
		if(this.x > game_sprites_Car.X_MAX - this.width) {
			this.x = game_sprites_Car.X_MAX - this.width;
		}
	}
	,__class__: game_sprites_Car
});
var game_sprites_NpcCar = function(x,y,graphic) {
	game_sprites_Car.call(this,x,y,graphic);
};
$hxClasses["game.sprites.NpcCar"] = game_sprites_NpcCar;
game_sprites_NpcCar.__name__ = true;
game_sprites_NpcCar.__super__ = game_sprites_Car;
game_sprites_NpcCar.prototype = $extend(game_sprites_Car.prototype,{
	update: function() {
		game_sprites_Car.prototype.update.call(this);
		if(this.y > 320) {
			this.kill();
		}
	}
	,init: function(velocity,velocityMax,velocityStep) {
		this.updateVelocity(velocity,velocityMax,velocityStep);
		var x = kha_math_Random.Default.GetFloatIn(this.width,240 - 2 * this.width);
		var y = -this.height;
		this.x = x;
		this.y = y;
		this.direction.x = (this.x < 120.?1:-1) * kha_math_Random.Default.GetFloatIn(0,0.5);
		this.direction.y = kha_math_Random.Default.GetFloatIn(0.5,1);
		this.direction.set_length(1);
		this.revive();
	}
	,updateVelocity: function(velocity,velocityMax,velocityStep) {
		this.velocity = velocityMax - velocity + velocityStep;
	}
	,__class__: game_sprites_NpcCar
});
var game_sprites_PlayerCar = function(graphic) {
	game_sprites_Car.call(this,0,0,graphic);
};
$hxClasses["game.sprites.PlayerCar"] = game_sprites_PlayerCar;
game_sprites_PlayerCar.__name__ = true;
game_sprites_PlayerCar.__super__ = game_sprites_Car;
game_sprites_PlayerCar.prototype = $extend(game_sprites_Car.prototype,{
	updateMovement: function(velocity,direction) {
		this.velocity = velocity;
		this.direction.x = direction;
	}
	,__class__: game_sprites_PlayerCar
});
var game_tools_Pool = function(capacity) {
	game_tools_Group.call(this);
	this.capacity = capacity;
};
$hxClasses["game.tools.Pool"] = game_tools_Pool;
game_tools_Pool.__name__ = true;
game_tools_Pool.__super__ = game_tools_Group;
game_tools_Pool.prototype = $extend(game_tools_Group.prototype,{
	capacity: null
	...
	,add: function(o) {
		if(this.elements.length >= this.capacity) {
			throw new js__$Boot_HaxeError("Pool at capacity");
		}
		this.capacity += 1;
		return game_tools_Group.prototype.add.call(this,o);
	}
	...
	,__class__: game_tools_Pool
});
var game_tools_ui_Text = function(font,size) {
	game_tools_Object.call(this,0,0,0,0);
	this.font = font;
	this.set_size(size);
	this.set_value("");
};
$hxClasses["game.tools.ui.Text"] = game_tools_ui_Text;
game_tools_ui_Text.__name__ = true;
game_tools_ui_Text.__super__ = game_tools_Object;
game_tools_ui_Text.prototype = $extend(game_tools_Object.prototype,{
	font: null
	,size: null
	,value: null
	,draw: function(g) {
		g.set_font(this.font);
		g.set_fontSize(this.size);
...
var kha_ImageList = function() {
	this.road_segmentDescription = { files : ["road_segment.png"], original_height : 32, type : "image", original_width : 200, name : "road_segment"};
	this.road_segmentName = "road_segment";
	this.road_segment = null;
	this.car_smallDescription = { files : ["car_small.png"], original_height : 32, type : "image", original_width : 16, name : "car_small"};
	this.car_smallName = "car_small";
	this.car_small = null;
	this.car_bigDescription = { files : ["car_big.png"], original_height : 32, type : "image", original_width : 32, name : "car_big"};
	this.car_bigName = "car_big";
	this.car_big = null;
};
...
kha_ImageList.prototype = {
	car_big: null
	,car_bigName: null
	,car_bigDescription: null
	,car_bigLoad: function(done) {
		kha_Assets.loadImage("car_big",function(image) {
			done();
		});
	}
	,car_bigUnload: function() {
		this.car_big.unload();
		this.car_big = null;
	}
	,car_small: null
	,car_smallName: null
	,car_smallDescription: null
	,car_smallLoad: function(done) {
		kha_Assets.loadImage("car_small",function(image) {
			done();
		});
	}
	,car_smallUnload: function() {
		this.car_small.unload();
		this.car_small = null;
	}
	,road_segment: null
	,road_segmentName: null
	,road_segmentDescription: null
	,road_segmentLoad: function(done) {
		kha_Assets.loadImage("road_segment",function(image) {
			done();
		});
	}
	,road_segmentUnload: function() {
		this.road_segment.unload();
		this.road_segment = null;
	}
...
kha_Kravur.prototype = {
	...
	,_get: function(fontSize) {
		if(!this.images.h.hasOwnProperty(fontSize)) {
			var width = 64;
			var height = 32;
			var baked = new Array(224);
			var _g1 = 0;
			var _g = baked.length;
			while(_g1 < _g) baked[_g1++] = new kha_graphics2_truetype_Stbtt_$bakedchar();
			var pixels = null;
			var status = -1;
			while(status < 0) {
				if(height < width) {
	...
	request.onreadystatechange = function() {
		...
		if(request.status >= 200 && request.status < 400) {
			...
			if(arrayBuffer != null) {
				...
				haxe_Log.trace("Error loading " + desc.files[0],{ fileName : "LoaderImpl.hx", lineNumber : 113, className : "kha.LoaderImpl", methodName : "loadBlobFromDescription"});
				window.console.log("loadBlob failed");
			}
			done(new kha_internal_BytesBlob(bytes));
		} else {
			haxe_Log.trace("Error loading " + desc.files[0],{ fileName : "LoaderImpl.hx", lineNumber : 119, className : "kha.LoaderImpl", methodName : "loadBlobFromDescription"});
			window.console.log("loadBlob failed");
		}
	};
	request.send(null);
...
kha_SystemImpl.initEx = function(title,options,windowCallback,callback) {
	haxe_Log.trace("initEx is not supported on the html5 target, running init() with first window options",{ fileName : "SystemImpl.hx", lineNumber : 72, className : "kha.SystemImpl", methodName : "initEx"});
...
kha_SystemImpl.loadFinished = function() {
	...
	try {
		...
		haxe_Log.trace(e,{ fileName : "SystemImpl.hx", lineNumber : 266, className : "kha.SystemImpl", methodName : "loadFinished"});
	}
...
kha_audio2_Audio1._mix = function(samples,buffer) {
	...
	while(_g4 < _g11.length) {
		...
		while(_g31 < _g21) {
			var i2 = _g31++;
			var _g41 = i2;
			kha_audio2_Audio1.sampleCache2[_g41] = kha_audio2_Audio1.sampleCache2[_g41] + kha_audio2_Audio1.sampleCache1[i2] * channel.get_volume();
		}
	}
	var _g5 = 0;
	...
	while(_g5 < _g12.length) {
		...
		while(_g32 < _g22) {
			var i3 = _g32++;
			var _g42 = i3;
			kha_audio2_Audio1.sampleCache2[_g42] = kha_audio2_Audio1.sampleCache2[_g42] + kha_audio2_Audio1.sampleCache1[i3] * channel1.get_volume();
		}
	}
	var _g13 = 0;
	var _g6 = samples;
...
kha_graphics2_truetype_StbTruetype.stbtt_BakeFontBitmap = function(data,offset,pixel_height,pixels,pw,ph,first_char,num_chars,chardata) {
	...
	while(_g11 < num_chars) {
		...
		var gw;
		var gh;
		var g = kha_graphics2_truetype_StbTruetype.stbtt_FindGlyphIndex(f,first_char + i);
...
var kha_graphics4_BlendingFactor = $hxClasses["kha.graphics4.BlendingFactor"] = { __ename__ : true, __constructs__ : ["Undefined","BlendOne","BlendZero","SourceAlpha","DestinationAlpha","InverseSourceAlpha","InverseDestinationAlpha"] };
...
kha_graphics4_TextShaderPainter.prototype = {
	...
	,drawString: function(text,opacity,color,x,y,transformation) {
		...
		while(_g1 < _g) {
			var q = font.getBakedQuad(this.charCodeAt(_g1++) - 32,xpos,y);
			if(q != null) {
...
game_SpeedRace.HEIGHT = 320;
game_SpeedRace.WIDTH = 240;
game_SpeedRace.maxCars = 3;
game_SpeedRace.velocityMax = 6;
game_SpeedRace.velocityStep = 2;
game_groups__$Road_RoadSegment.v = 0;
game_sprites_Car.X_MIN = 0;
game_sprites_Car.X_MAX = 240;
...
kha_Scheduler.lastNow = 0;
kha_Shaders.painter_colored_fragData = "s206:I3ZlcnNpb24gMTAwCi8vIFVua25vd24gZXhlY3V0aW9uIG1vZGUgOApwcmVjaXNpb24gbWVkaXVtcCBmbG9hdDsKdmFyeWluZyB2ZWM0IGZyYWdtZW50Q29sb3I7CgoKCnZvaWQgbWFpbigpCnsKCWdsX0ZyYWdDb2xvciA9IGZyYWdtZW50Q29sb3I7CglyZXR1cm47Cn0KCg";
kha_Shaders.painter_colored_vertData = "s426:";
kha_Shaders.painter_image_fragData = "s658:";
kha_Shaders.painter_image_vertData = "s506:";
kha_Shaders.painter_text_fragData = "s574:";
kha_Shaders.painter_text_vertData = "s527:";
kha_Shaders.painter_video_fragData = "s658:";
